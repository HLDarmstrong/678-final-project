---
title: "Final_Project_NBA"
author: "Shengbo Wang"
date: "2022-12-05"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
	echo = F,
	message = FALSE,
	warning = FALSE
)
library(tidyr)
library(ggplot2)
library(tidyverse)
library(dplyr)
library(knitr)
library(stringr)
library(kableExtra)
library(lme4)
library(corrplot)
library(PerformanceAnalytics)
library(latexpdf)
library(jtools)
```

##Abstract


##Introduction

I knew Yao Ming when I was very young. He is the most famous Chinese player in NBA in China till today. Yao Ming has also brought the game of basketball to schools. I also started watching NBA games because of Yao Ming. Now Basketball is one of the most popular sports in the world, NBA has the most high quality of players. Some of the players are over 34 millions per year. Apart from the players' popularity and Internet traffic, I want to know what makes more money per year by looking at their statistic in the game. There are five positions in Basketball, each position will have different ability to help their team to win. And each teams will have different tactics like the Golden State Warriors are famous the three points, so they might hire better three-point shooter in higher salary. So, I decided make a multilevel model to find the influence of fixed effects.

##Methods


###Data Preprocessing

The data set come from (https://www.kaggle.com/datasets/drgilermo/nba-players-stats), which has the Season stats from many different aspects, such ass Three point field goals, two points field goals, win shares, etc. And I found the (https://www.kaggle.com/datasets/koki25ando/salary) which has the salary of NBA players during the season 2017-2018, so just filter the year in the Season stats in 2016 because the salary is based on the players' last season performance, and inner join with the salary data, finally I got 251 observations in 13 variables. There are five positions in Basketball, each position will have different ability to help their team to win. And each teams will have different tactics like the Golden State Warriors are famous the three points. I decided make a multilevel model to find the influence of fixed effects.


| column names    | explanation                                         |
| :--:            | :-----                                              |
| Player          | Name of the player                                  |
| Tm              | Name of the Team                                    |
| Pos             | Position of player                                  |
| Age             | Age of player                                       |
| G               | Game played                                         |
| PER             | Player Efficiency Rating                            |
| WSpWG           | Win Shares Per 48 Minutes                           |
| FG              | Field Goals Per Game                                |
| ThreeP          | 3-Point Field Goals Per Game                        |
| TwoP            | 2-Point Field Goals Per Game                        |
| FT              | Free Throws Per Game                                |
| TRB             | Rebounds Per Game                                   |
| AST             | Assists Per Game                                    |
| STL             | Steal Per Game                                      |
| BLK             | Blocks Per Game                                     |
| TOV             | Turnovers Per Game                                  |
| PF              | Personal Fouls Per Game                             |
| PTS             | Points  Per Game                                    |
| Salary          | Annual salary of player (dollars)                   |

```{r}
Salary <- read_csv("NBA_season1718_salary.csv") %>% 
  select(Player,Tm,Salary=salary)
Seasons_Stats <- read_csv("Seasons_Stats.csv") %>%
  filter(Year==2016) %>%
  select(Player,Pos,Age,Tm,G,PER,
         WSpWG="WS/48",FG,ThreeP="3P",TwoP="2P",
         FT,TRB,AST,STL,BLK,TOV,PF,PTS)

raw <- inner_join(Seasons_Stats,Salary)
NBA <- raw %>% 
  mutate(FG=(FG/G),ThreeP=(ThreeP/G),TwoP=(TwoP/G),
         FT=(FT/G),TRB=(TRB/G),AST=(AST/G),STL=(STL/G),
         BLK=(BLK/G),TOV=(TOV/G),PF=(PF/G),PTS=(PTS/G))

set.seed(0611)
sample_team <- sample(unique(NBA$Tm), 15, replace = FALSE)
```


```{r}
ggplot(subset(NBA,Tm %in% sample_team),aes(Salary))+
  geom_density(col="red")+
  facet_wrap(~Tm)

ggplot(NBA,aes(Salary))+
  geom_density(col="blue")+
  facet_wrap(~Pos)
```

##EDA

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Player Efficiency Rating and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = log(PER + 1), y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = log(PER + 1), y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Points per Game made and salaries")}

ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = log(PTS + 1), y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = log(PTS + 1), y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Win share per 48 mins and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = WSpWG, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = WSpWG, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```
```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Three-point per Game and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = ThreeP, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = ThreeP, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')

```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Two-point per Game and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = TwoP, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = TwoP, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')

```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Field goals per Game and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = FG, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = FG, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')

```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Rebound per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = TRB, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = TRB, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```
```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Assiants  per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = AST, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = AST, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Steals  per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = STL, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = STL, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Blocks  per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = BLK, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = BLK, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Turnovers  per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = TOV, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = TOV, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```

```{r, fig.show="hold", out.width="50%",fig.cap=paste("Realationship between Personal fouls per games and salaries")}
ggplot(subset(NBA,Tm %in% sample_team)) + 
  aes(x = PF, y = log(Salary)) + 
  geom_point(aes(color = factor(Tm)), size = .6) + 
  geom_smooth(aes(color = factor(Tm)), method = "lm", se = FALSE, formula = 'y ~ x')

ggplot(NBA) + 
  aes(x = PF, y = log(Salary)) + 
  geom_point(aes(color = factor(Pos)), size = .6) + 
  geom_smooth(aes(color = factor(Pos)), method = "lm", se = FALSE, formula = 'y ~ x')
```


#Model fitting

```{r, fig.height=5, fig.width=7, fig.cap="Correlation Matrix"}
NBA_Subset <- NBA[,c(6:18)]
chart.Correlation(cor(NBA_Subset))
```
I will pick the predictors based on the correlation plot, I will keep the PTS, STL, AST, BLK, TOV and TRB, because they are the most important stats in basketball, For the other groups with high correlation like WSpWG and PER(0.88), FT and TOV(0.81), FG are having high correlations with many factors, So I decide to remove PER, FT and FG to fit the model.

```{r}
model=lmer(log(Salary)~PTS+STL+AST+BLK+TOV+TRB+PF+ThreeP+TwoP+
             (1|Tm)+
             (1|Pos),
           data=NBA)
summary(model)
```

```{r}
summ(model)
```

