...
PTS: The Salary per year will increase 0.54% as the points per game increase 1%; The salary per year will increase 0.83% as the rebound per game increase 1%; The salary per year will increase 0.63% as the Three points increase 1%; The salary per year will increase 2.68% as the Win share per year increase 1%.
Each team 
For a example to add a random effect who is PG in DAL, the formula will be $$log(Salary+1)=(12.4328+0.0674) +(0.5397+0.1109)*log(PTS+1)+(0.8288-0.1479) *log(TRB+1)+(0.6317-0.1322)*log(ThreeP+1)+2.6771*log(WSpWG+1)$$
For each increase 1% of player's point, the salary is 0.779% for DAL's point guard. Each coefficient represent each preference in each type of statistics.
